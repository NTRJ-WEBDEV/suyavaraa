...
import { CreateSuyamvaramForm } from '@/components/suyamvaram/CreateSuyamvaramForm'

export default async function CreateSuyamvaramPage() {
  ...
  return (
    <AppShell user={{ name: profile?.full_name ?? null, photo: up?.primary_photo_url ?? null }} initialMode="matrimony">
      <CreateSuyamvaramForm userId={user.id} />
    </AppShell>
  )
}
